/*
    This sketch is for creating an Ebook using AMB82-mini and SD Card.
    ...
    Example guide:
    https://ameba-doc-arduino-sdk.readthedocs-hosted.com/en/latest/ameba_pro2/amb82-mini/Example_Guides/E-Paper/7.5-inch%20e-Book%20with%20MicroSD%20card.html
 */
...
char buffer0[PAGESIZE];
char buffer1[200][200];
...
void read_page() {
    ...
    if (file.available()) {
        ...
        pos = file.position();
        paint.LongTextWrap(buffer0, string, fontsize, rotate);

        int word_counter = 0;
        int line_counter = 0;
        for (unsigned int i = 0; i <= (strlen(buffer0)); i++) {
            if (buffer0[i] == '\n') {
                buffer1[line_counter][word_counter] = '\0';
                line_counter++;
                word_counter = 0;
            } else {
                buffer1[line_counter][word_counter] = buffer0[i];
                word_counter++;
            }
        }
        ...
        for (int i = 0; i <= line_counter; i++) {
            if (buf_size - 1 == PAGESIZE - 1) {
                if (i == line_counter) {
                    int last_line_len = strlen(buffer1[line_counter]);
        ...
        if (buf_size - 1 == PAGESIZE - 1) {
            if (fontsize == 16) {
                ...
                for (int i = 0; i < line_counter; i++) {
                    paint.DrawStringAt(0, k, buffer1[i], &Font16, COLOR_BLACK);
                    k += spacing;
                }
            } else if (fontsize == 20) {
                if (rotate == 1 || rotate == 3) {
                ...
                for (int i = 0; i < line_counter; i++) {
                    paint.DrawStringAt(0, k, buffer1[i], &Font20, COLOR_BLACK);
                    k += spacing;
                }
            } else if (fontsize == 24) {
                if (rotate == 1 || rotate == 3) {
                ...
                for (int i = 0; i < line_counter; i++) {
                    paint.DrawStringAt(0, k, buffer1[i], &Font24, COLOR_BLACK);
                    k += spacing;
                }
            }
