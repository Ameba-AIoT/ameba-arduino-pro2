...
void upload_process(void) {
    if (check_flash_erase == 100) {
        cmdss.clear();
        cmdss << image_tool_folder_name << flash_tool_name << " -p " << serial_port <<" -b " << flash_speed << " -e chip -r ";
        getline(cmdss, cmd);
        //cout << cmd << endl;
        system(cmd.c_str());
        check_flash_erase = 0;
    ...
    if (check_image_upload == 100) {
        if (check_upload_mode == 0x110) {
            cmdss.clear();
            cmdss << image_tool_folder_name << flash_tool_name << " -p " << serial_port << " -f firmware_isp_iq.bin -b " << flash_speed << " -s " << VOE_address_PT_ISP_IQ << " -U -r ";
            getline(cmdss, cmd);
            system(cmd.c_str());
        } else if (check_upload_mode == 0x101) {
            cmdss.clear();
            cmdss << image_tool_folder_name << flash_tool_name << " -p " << serial_port << " -f nn_model.bin -b " << flash_speed << " -s " << NN_address_PT_NN_MDL << " -U -r ";
            getline(cmdss, cmd);
            system(cmd.c_str());
        } else if (check_upload_mode == 0x100) {
            cmdss.clear();
            cmdss << image_tool_folder_name << flash_tool_name << " -p " << serial_port << " -f flash_ntz.bin -b " << flash_speed << " -s " << fw1_address << " -U -r ";
            getline(cmdss, cmd);
            system(cmd.c_str());
        } else {
            cmdss.clear();
            cmdss << image_tool_folder_name << flash_tool_name << " -p " << serial_port << " -f flash_ntz.bin -b " << flash_speed << " -U -r ";
            getline(cmdss, cmd);
            system(cmd.c_str());
        }
   ...
   if (upload_mode_user_selection == "SpeedEnable") {
        ...
        if (upload_mode_user_selection_voe == "VOEyes") {
            ...
            if (upload_mode_user_selection_nn == "NNyes") {
                ...
                cout << "    Normal Mode, Neural Network requires the Video ISP to function. Continuing with both bins enabled. " << endl;
            } else {
                check_upload_mode = 0x0;
            }
        }
