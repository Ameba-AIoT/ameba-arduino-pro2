...
void cameraUpdateParams(mm_context_t *p, void *p_priv, int stream_id, int type, int res, int w, int h, int bps, int fps, int gop, int rc_mode, int snapshot, int jpeg_qlevel, int video_rotation){ 
    ...
    if(p) {
        ...
        if(mm_module_ctrl(p, CMD_VIDEO_STREAM_STOP, video_params.stream_id) == -1) printf("[cameraUpdateParams] CTRL CMD_VIDEO_STREAM_STOP HAS FAILED\r\n"); 
        if(mm_module_ctrl(p, CMD_VIDEO_SET_PARAMS, (int)&video_params) == -1) printf("[cameraUpdateParams] CTRL CMD_VIDEO_SET_PARAMS HAS FAILED\r\n");
        if(mm_module_ctrl(p, CMD_VIDEO_APPLY, video_params.stream_id) == -1) printf("[cameraUpdateParams] CTRL CMD_VIDEO_APPLY HAS FAILED\r\n");
    }
