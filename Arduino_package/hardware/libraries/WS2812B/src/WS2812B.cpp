...
void WS2812B::begin(void)
{
    ...
    if (_input_pin == SPI_MOSI) {
        ...
        // TBD
        // (PinName)g_APinDescription[SPI_MISO].pinname
        // (PinName)g_APinDescription[SPI_SCLK].pinname
        // (PinName)g_APinDescription[SPI_SS].pinname
    } else if (_input_pin == SPI1_MOSI) {
        _SPI = &SPI1;
        // Revert the unnecessary SPI pins to GPIO functions
        // TBD
        // (PinName)g_APinDescription[SPI1_MISO].pinname
        // (PinName)g_APinDescription[SPI1_SCLK].pinname
    ...
    _SPI->setDefaultFrequency(2500000);
    _SPI->setDataMode(12, 0);
    _SPI->begin();
}
