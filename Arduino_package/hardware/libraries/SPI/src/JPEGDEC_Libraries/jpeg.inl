            ...
            for (iBitNum = 1; iBitNum <= 16; iBitNum++) {
                ...
                while (iLen) {
                    ...
                    if (ucCode != 0 && (ucCode + iBitNum) <= 6 && pJPEG->ucMode != 0xc2)    // we can fit the magnitude value in the code lookup (not for progressive)
                    {
                        ...
                        repeat = 1 << ucMag;
                        iLoop = 1 << (count - ucMag);
                        for (j = 0; j < repeat; j++) {     // calculate the magnitude coeff already
    ...
    if (pJPEG->b11Bit)    // 11-bit "slow" tables used
    {
        ...
        while (pZig < pEnd) {
            ...
            if (pZig < pEnd && usHuff)    // && piHisto)
            {
                ...
                if (*pZig >= 0x20) {
                    u16MCUFlags |= 0x100;    // bottom 4 rows
                }
...
// full size
    ...
    if (pJPEG->ucPixelType == RGB8888) {
        ...
        for (iRow = 0; iRow < 8; iRow++) {           // do 8 rows
            ...
            i168Crx2 = vzipq_s16(i168Cr, i168Cr);    // double elements in horizontal direction
