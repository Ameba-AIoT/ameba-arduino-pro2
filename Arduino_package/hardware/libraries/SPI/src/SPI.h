...
public:
	...
	byte transfer(byte _pin, uint8_t _data, SPITransferMode _mode = SPI_LAST); // for transferring 1 byte data with slave select
	byte transfer (uint8_t _data, SPITransferMode _mode = SPI_LAST); // for transferring 1 byte data
	...
	uint16_t transfer16(uint16_t _data, SPITransferMode _mode = SPI_LAST); // for transferring 2 byte data
