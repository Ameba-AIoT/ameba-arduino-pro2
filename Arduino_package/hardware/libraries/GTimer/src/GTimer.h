...
public:
     ...
     * @param[in] periodical By default, the timer would keep periodically countdown and reload leads the handler periodically invoked.
     * @param[in] userdata The user data brings to the handler
     * @param[in] timer0_clk_sel Select the timer0 clock, 0 for 40MHz or 1 for 4MHz
     */
     ...
     */
    long long read_us(uint32_t timerid);
};
