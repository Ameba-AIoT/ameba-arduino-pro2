...
static const const char *err_str[] = {
    "no error",                /* OK */
    "out of memory",           /* NOMEM */
                               ...
                               int verbosity)
{
    ...
    if (err->detail) {
        ...
        if (strstr(err->detail, "%s")) {
            ...
            if (err->buf) {
                ...
                err->buf = (char *)malloc(newlen + strlen(err->arg_str));
            }
    ...
    if (err->type == ZBAR_ERR_SYSTEM) {
        ...
        if (err->buf) {
            ...
            err->buf = (char *)malloc(len + strlen(sysfmt) + strlen(syserr));
        }
        ...
        if (FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_IGNORE_INSERTS,
            ...
            if (err->buf) {
                ...
                err->buf = malloc(len + strlen(sysfmt) + strlen(syserr));
            }
            len += sprintf(err->buf + len, sysfmt, syserr, err->errnum);
            LocalFree(syserr);
        }
