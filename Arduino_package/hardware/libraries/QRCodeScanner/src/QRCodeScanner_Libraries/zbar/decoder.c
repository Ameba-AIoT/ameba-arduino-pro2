...
#include "config.h"
#include "qr_code_stdlib.h" /* malloc, calloc, free */

#include "zbar.h"
#include "decoder.h"

#if defined(DEBUG_DECODER) || defined(DEBUG_EAN) || defined(DEBUG_CODE39) || defined(DEBUG_I25) || defined(DEBUG_CODE128) || defined(DEBUG_QR_FINDER) || (defined(DEBUG_PDF417) && (DEBUG_PDF417 >= 4))
...
{
    zbar_decoder_t *dcode = (zbar_decoder_t *)malloc(1 * sizeof(zbar_decoder_t));
    memset(dcode, 0, 1 * sizeof(zbar_decoder_t));
...
#ifdef ENABLE_EAN
    dcode->ean.enable = TEST_CFG(dcode->ean.ean13_config | dcode->ean.ean8_config | dcode->ean.upca_config | dcode->ean.upce_config | dcode->ean.isbn10_config | dcode->ean.isbn13_config,
                                 ZBAR_CFG_ENABLE);
#endif
