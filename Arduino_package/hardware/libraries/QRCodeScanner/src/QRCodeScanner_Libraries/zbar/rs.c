...
int main(void)
{
    ...
    for (;;) {
        ...
        int nerrors;
        int i;
        if (scanf("%i", &ndata) < 1 || ndata < 0 || ndata > 255 || scanf("%i", &npar) < 1 || npar < 0 || npar > ndata) {
            break;
        }
        for (i = 0; i < ndata; i++) {
        ...
        for (i = 0; i < nerasures; i++) {
            if (scanf("%i", ierasures + i) < 1 || ierasures[i] < 0 || ierasures[i] >= ndata) {
                break;
            }
            ...
            for (c = 0; c < 256; c++) {
                for (d = 0; d < 256; d++) {
                    ...
                    for (j = 1; j < 256; j++) {
                        ...
                        e[mroots] = d ^ rs_hgmul(&gf, c, logx) ^ rs_hgmul(&gf, b, logx2) ^ rs_hgmul(&gf, a, gf.log[gf.exp[logx + logx2]]) ^ gf.exp[logx2 << 1];
                        if (!e[mroots]) {
                            r[mroots++] = j;
                        }
                    }
