...
#include <inttypes.h>
#endif
#include "qr_code_stdlib.h" /* malloc, free */
                              ...
                              int datalen)
{
    ...
    if (sym) {
        ...
        sym = (zbar_symbol_t *)malloc(1 * sizeof(zbar_symbol_t));
        memset(sym, 0, 1 * sizeof(zbar_symbol_t));
        STAT(sym_new);
    }
                                          ...
                                          zbar_symbol_t *sym)
{
    ...
    while (*entry) {
        if ((*entry)->type == sym->type && (*entry)->datalen == sym->datalen && !memcmp((*entry)->data, sym->data, sym->datalen)) {
            break;
        }
...
static inline void qr_handler(zbar_image_scanner_t *iscn)
{
...
#if OPTIMIZE_TIME == 1
    line->boffs_l = u - zbar_scanner_get_edge(iscn->scn, line->boffs_l, QR_FINDER_SUBPREC);
#endif
    ...
    line->eoffs = zbar_scanner_get_edge(iscn->scn, line->eoffs,
                  ...
                  - line->len;
#if OPTIMIZE_TIME == 1
    line->eoffs_r = zbar_scanner_get_edge(iscn->scn, line->eoffs_r,
                                          QR_FINDER_SUBPREC)
                    - line->len;
#endif
    line->len -= u;
...
static void symbol_handler(zbar_decoder_t *dcode)
{
    ...
    for (sym = iscn->syms->head; sym; sym = sym->next) {
        if (sym->type == type && sym->datalen == datalen && !memcmp(sym->data, data, datalen)) {
            sym->quality++;
            if (TEST_CFG(iscn, ZBAR_CFG_POSITION)) {
...
zbar_image_scanner_t *zbar_image_scanner_create(int x_density, int y_density)
{
    zbar_image_scanner_t *iscn = (zbar_image_scanner_t *)malloc(1 * sizeof(zbar_image_scanner_t));
    if (!iscn) {
        return (NULL);
    }
    memset(iscn, 0, 1 * sizeof(zbar_image_scanner_t));
    ...
    CFG(iscn, ZBAR_CFG_X_DENSITY) = x_density;
    CFG(iscn, ZBAR_CFG_Y_DENSITY) = y_density;
                    ...
                    zbar_image_t *img, int *scan_ret)
{
    ...
    iscn->time = (abstime.tv_sec * 1000) + (((abstime.tv_nsec / 500000) + 1) >> 1);
#else
    // struct timeval abstime;
    // gettimeofday(&abstime, NULL);
    // iscn->time = (abstime.tv_sec * 1000) + (((abstime.tv_usec / 500) + 1) >> 1);
#endif
...
#if OPTIMIZE_TIME == 1
    _zbar_find_lines_range(iscn->qr, 0, &minx, &miny, &maxx, &maxy, w, h);    // 0: scan x first; 1: scan y first
    // printf("zbar_scan_image density=%d minx=%d,miny=%d,maxx=%d,maxy=%d\r\n",density,minx,miny,maxx,maxy);
#endif
    ...
    if (density > 0) {
        ...
        const uint8_t *p = data;
        int x = 0, y = 0;
        int border = (((w - 1) % density) + 1) >> 1;
        if (border > (w >> 1)) {
            border = w >> 1;
        }
#if OPTIMIZE_TIME == 1
        border += minx;
        movedelta(border, miny);
#else
        movedelta(border, 0);
#endif
        iscn->v = x;
...
#ifdef ENABLE_QRCODE
    *scan_ret = _zbar_qr_decode(iscn->qr, iscn, img);
#endif

    /* FIXME tmp hack to filter bad EAN results */
    if (syms->nsyms && !iscn->enable_cache && (density == 1 || CFG(iscn, ZBAR_CFG_Y_DENSITY) == 1)) {
        zbar_symbol_t **symp = &syms->head, *sym;
        while ((sym = *symp)) {
            if (sym->type < ZBAR_I25 && sym->type > ZBAR_PARTIAL && sym->quality < 3) {
                /* recycle */
                *symp = sym->next;
                syms->nsyms--;
                sym->next = NULL;
