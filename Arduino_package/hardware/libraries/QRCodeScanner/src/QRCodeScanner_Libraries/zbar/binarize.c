...
/*The above algorithms are computationally expensive, and do not work as well
  ...
  However, it also tends to over-shrink isolated black dots inside the code,
   making them easy to miss with even slight misalignment.
...
void merge_image(unsigned char *sour_img, int sour_width, int sour_height, int merge_height, unsigned char *dest_img, int dest_width, int dest_height, int start_x, int start_y)
{
    int x = 0, y = 0;
    int mask_start_y = 0;
    int x_byte_index = 0, x_bit_index = 0;
    unsigned char temp_mask = 0, bit = 0, data = 0;

    if (merge_height > sour_height) {
        printf("merge_image error !merge_height(%d) > sour_height(%d)\n", merge_height, sour_height);
    }

    if (start_y % sour_height == 0) {    // merge 1/2 mask each time
        mask_start_y = 0;                // start_y=0/256/512 merge the up half of mask
    } else {
        mask_start_y = sour_height >> 1;    // start_y=128/384 merge the down half of mask
    }

    // printf("merge_image start_x=%d,start_y=%d,mask_start_y=%d sour_height=%d merge_height=%d\n",start_x,start_y,mask_start_y,sour_height,merge_height);
#if OPTIMIZE_MEM_SIZE == 1
    for (x = 0; x < sour_width; x++) {
        for (y = 0; y < merge_height; y++) {
            dest_img[(y + start_y) * dest_width + x + start_x] = sour_img[(y + mask_start_y) * sour_width + x];
        }
    }
#elif OPTIMIZE_MEM_SIZE == 2
    for (y = 0; y < merge_height; y++) {
        for (x = 0; x < sour_width; x++) {
            temp_mask = sour_img[(y + mask_start_y) * sour_width + x];
            for (x_bit_index = 0; x_bit_index < 8 && x * 8 + x_bit_index < dest_width; x_bit_index++) {
                data = ((temp_mask >> x_bit_index) & 0x1 == 1) ? 0xFF : 0;
                dest_img[(y + start_y) * dest_width + start_x + x * 8 + x_bit_index] = data;
                // printf("merge_image x=%d,y=%d,x_bit_index=%d dest_img[%d]=%d\n",x,y,x_bit_index,(y+start_y)*dest_width+start_x+x*8+x_bit_index,dest_img[(y+start_y)*dest_width+start_x+x*8+x_bit_index]);
            }
        }
    }
#endif
...
unsigned char *qr_binarize(const unsigned char *_img, int _width, int _height)
{
    ...
    if (_width > 0 && _height > 0) {
        ...
        int x;
        int y;
        int merge_x = 0, merge_y = 0, update_x = 0, update_y = 0, next_y = 0, realmaskheight = 0;
        int x_byte_index = 0, x_bit_index = 0, mask_width = 0, mask_height = 0;
        unsigned char temp_mask = 0, bit = 0;
        ...
        windw = 1 << logwindw;
        windh = 1 << logwindh;
        // printf("qr_binarize:windw=%d, windh=%d logwindw=%d logwindh=%d\r\n",windw,windh,logwindw,logwindh);

#if OPTIMIZE_MEM_SIZE == 1
        mask_width = _width;
        mask_height = windh;
#elif OPTIMIZE_MEM_SIZE == 2
        mask_width = (_width + 7) >> 3;    // 1Byte can store 8 Bit
        mask_height = windh;
#else
        mask_width = _width;
        mask_height = _height;
#endif
        mask = (unsigned char *)malloc(mask_width * mask_height * sizeof(*mask));
        ...
        for (y = 0; y < _height; y++) {
            ...
            for (x = 0; x < _width; x++) {
                ...
                g = _img[y * _width + x];
#if (OPTIMIZE_MEM_SIZE == 1)
                mask[((y % windh) * _width + x)] = -(g + 3 << logwindw + logwindh < m) & 0xFF;
#elif (OPTIMIZE_MEM_SIZE == 2)
                x_byte_index = x >> 3;    //  mean /8
                x_bit_index = x & 0x7;    // mean %8
                temp_mask |= ((((g + 3 << logwindw + logwindh < m) == 0) ? 0 : 1) << x_bit_index);
                if (x_bit_index == 7 || x == _width - 1) {
                    mask[(y % windh) * mask_width + x_byte_index] = temp_mask;
                    temp_mask = 0;
                    // printf("x_byte_index=%d x_bit_index=%d bit=%d,mask[%d]=0x%x,(g+3<<logwindw+logwindh<m)=0x%x\n",x_byte_index,x_bit_index,bit, (y%windh)*mask_width+x_byte_index, mask[(y%windh)*mask_width+x_byte_index],(g+3<<logwindw+logwindh<m));
                }
#else
...
#if OPTIMIZE_MEM_SIZE != 0
            next_y = y + 1;
            if (next_y >= windh && next_y % (windh >> 1) == 0 || next_y == _height)    // for example, when y=256 , merge back row 0~127 ; y=384, merge back row 128~255
            {
                if (next_y == _height) {    // last rows
                    // merge_x=0;
                    merge_y += windh >> 1;
                    realmaskheight = windh >> 1;
                    merge_image(mask, mask_width, mask_height, realmaskheight, (unsigned char *)_img, _width, _height, merge_x, merge_y);

                    // merge_x=0;
                    merge_y += windh >> 1;
                    realmaskheight = _height - merge_y;

                } else {    // Integer multiple of windh
                    // merge_x=0;
                    merge_y = next_y - windh;
                    realmaskheight = windh >> 1;
                }
...
#if OPTIMIZE_MEM_SIZE != 0
        image_write_png(_img, _width, _height, "binary.png");
#else
        image_write_png(mask, _width, _height, "binary.png");
#endif
        // fclose(fout);
    }
#endif
#if OPTIMIZE_MEM_SIZE != 0
    return (unsigned char *)(_img);
#else
    return (mask);
#endif
}
