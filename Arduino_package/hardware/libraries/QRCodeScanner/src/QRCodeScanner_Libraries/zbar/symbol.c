                      ...
                      unsigned *len)
{
    ...
    unsigned maxlen = (strlen(xmlfmt[0]) + strlen(xmlfmt[1]) + strlen(xmlfmt[2]) + strlen(xmlfmt[3]) + strlen(type) + datalen + MAX_INT_DIGITS + 1);
    if (!*buf || (*len < maxlen)) {
        if (*buf) {
            free(*buf);
        }
...
{
    zbar_symbol_set_t *syms = (zbar_symbol_set_t *)malloc(1 * sizeof(*syms));
    memset(syms, 0, 1 * sizeof(*syms));
