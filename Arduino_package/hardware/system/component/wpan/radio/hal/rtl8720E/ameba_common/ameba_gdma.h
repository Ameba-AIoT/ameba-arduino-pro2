...
#define GDMA0_REG_BASE           0x40110000
#define GDMA_BASE           ((GDMA_TypeDef          *) (GDMA0_REG_BASE))
#define GDMA0_REG_BASE_S         0x50110000
#define ENABLE 1
#define DISABLE 0
#define NULL_IRQ 29
...
#define assert_param(VAL)   do {\
                                if (!(VAL)) DiagPrintf("%s(%d):Assert fail!\n", __FUNCTION__, __LINE__);\
                            }while(0)

static inline int TrustZone_IsSecure()
{
	return 0;
}

#define InterruptRegister(IrqFun, IrqNum, Data, Priority) do {\
															xt_set_interrupt_handler((IrqNum), (IrqFun), (void *)(Data), NULL);\
                                                            }while(0)
#define InterruptEn(IrqNum, IrqPriority) do {\
											xt_ints_on( ( 1 << (IrqNum) ) );\
                                        }while(0)

#define InterruptDis(IrqNum) do {\
                                xtos_interrupt_disable((IrqNum));\
                            }while(0)

#define InterruptUnRegister(IrqNum) do {\
                                        xtos_interrupt_clear((IrqNum));\
                                    }while(0)
...
enum DSPIRQn {
	...
	GDMA0_CHANNEL4_IRQ          = NULL_IRQ,
	GDMA0_CHANNEL5_IRQ          = NULL_IRQ,
	GDMA0_CHANNEL6_IRQ          = NULL_IRQ,
	GDMA0_CHANNEL7_IRQ          = NULL_IRQ,
};
...
__STATIC_INLINE void GDMA_BurstEnable(u32 ch_num, u32 NewState)
{
	...
	DiagPrintf("%s: CFG_HIGH = 0x%x\n", __FUNCTION__, ValTemp);
}
