...
typedef struct wlan_func_stubs_s {
	...
	void (*AES_WRAP)(unsigned char *plain, int plain_len, unsigned char *iv, int iv_len, unsigned char *kek, int kek_len, unsigned char *cipher,
	...
	void (*rom_psk_CalcGTK)(unsigned int AuthKeyMgmt, unsigned char *addr, unsigned char *nonce, unsigned char *keyin, int keyinlen, unsigned char *keyout,
							int keyoutlen);
	void (*rom_psk_CalcPTK)(unsigned int AuthKeyMgmt, unsigned char *addr1, unsigned char *addr2, unsigned char *nonce1, unsigned char *nonce2,
							unsigned char *keyin, int keyinlen, unsigned char *keyout, int keyoutlen);
#else
	...
	void (*rom_psk_CalcPTK)(unsigned char *addr1, unsigned char *addr2, unsigned char *nonce1, unsigned char *nonce2, unsigned char *keyin, int keyinlen,
...
#ifdef CONFIG_PLATFORM_8735B
	int (*sha256_prf_bits)(u8 *key, size_t key_len, char *label, u8 *data, size_t data_len, u8 *buf, size_t buf_len_bits);
	int (*sha256_prf)(u8 *key, size_t key_len, char *label,	u8 *data, size_t data_len, u8 *buf, size_t buf_len);
#endif
	// sha1
