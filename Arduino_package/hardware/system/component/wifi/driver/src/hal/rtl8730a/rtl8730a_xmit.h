...
#define GET_FIX_RATE_8730A(v)		((v) & 0x0FFF)
#define GET_FIX_RATE_SGI_8730A(v)	(((v) & 0x7000) >> 12)
...
s32 rtl8730ad_init_xmit_priv(PADAPTER padapter);
void rtl8730ad_free_xmit_priv(PADAPTER padapter);
s32	rtl8730ad_hal_xmit(_adapter *padapter, struct xmit_frame *pxmitframe);
s32 rtl8730ad_mgnt_xmit(_adapter *padapter, struct xmit_frame *pmgntframe);
s32	rtl8730ad_hal_xmitframe_enqueue(_adapter *padapter, struct xmit_frame *pxmitframe);
void rtl8730a_xmitframe_resume(_adapter *padapter);
u32 GetDmaTxbdIdx(u32 ff_hwaddr);
struct xmit_buf *rtl8730a_dequeue_xmitbuf(struct rtw_tx_ring	*ring);
