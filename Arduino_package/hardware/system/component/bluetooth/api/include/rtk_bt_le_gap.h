...
/**
 * @typedef   rtk_bt_le_adv_stop_casue_t
...
/**
 * @enum    rtk_bt_le_gap_dev_state_t
...
typedef struct {
    uint16_t conn_handle[GAP_MAX_LINKS];
    uint16_t conn_num;
 ...
 * @brief     Start disconnection, will casue event @ref RTK_BT_LE_GAP_EVT_DISCONN_IND
...
/**
 * @fn        uint16_t rtk_bt_le_gap_get_active_conn_num(uint16_t *p_active_num)
 * @brief     Get active connection number.
 * @param[out] p_active_num: Active connection number
 * @return    
 *            - 0  : Succeed
 *            - Others: Error code
 */
uint16_t rtk_bt_le_gap_get_active_conn_num(uint16_t *p_active_num);
