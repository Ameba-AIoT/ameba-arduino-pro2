...
/** @defgroup PERIPH_ANCS_Exported_Macros ANCS Exported Macros
   * @{
   */
#define BT_ANCS_MAX_ATTR_LEN 256 //!< Max ancs attribute length

#if F_BT_ANCS_GET_APP_ATTR
#define BT_ANCS_APP_INDENTIFIER_MAX_LEN 30  //!< Max app indentifier length
#endif

#define BT_ANCS_MSG_QUEUE_NUM 5  //!< ANCS message queue size
...
typedef struct
{
    ...
    uint8_t    data[BT_ANCS_MAX_ATTR_LEN];
...
{
    uint8_t  conn_id;
    char     app_identifier[BT_ANCS_APP_INDENTIFIER_MAX_LEN];
 ...
 * @return void
 */
void bt_ancs_init(uint8_t link_num);

void bt_ancs_deinit(void);
 ...
 * @return   void
 */
void bt_ancs_handle_msg(T_IO_MSG *p_io_msg);
