...
struct BT_MESH_PRIV {
    ...
    uint16_t processing_group;
    uint16_t group_binding_device;
    uint8_t target_group_light_state;
    uint16_t get_group_state_address;
...
void bt_mesh_cmd_set_node_group_pre_handler(uint16_t mesh_addr, uint16_t group_addr, char *group_name, uint8_t group_name_length);
void bt_mesh_cmd_set_node_group_handler(void);
void bt_mesh_cmd_set_node_group_post_handler(uint8_t *bt_mesh_event_value);

/* RTW_BT_CMD_SET_NODE_NAME */
void bt_mesh_cmd_set_node_name_pre_handler(uint8_t *dev_uuid, char *node_name, uint8_t node_name_length);
void bt_mesh_cmd_set_node_name_handler(void);
void bt_mesh_cmd_set_node_name_post_handler(uint8_t *bt_mesh_event_value);
