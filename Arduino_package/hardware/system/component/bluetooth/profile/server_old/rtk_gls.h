...
void glucose_srv_callback(rtk_bt_gatts_evt_code_t event, void *data);

rtk_bt_err_t gls_set_parameter(T_GLS_PARAM_TYPE param_type, uint8_t len, void *p_value);

rtk_bt_err_t gls_get_parameter(T_GLS_PARAM_TYPE param_type, uint8_t *len, void *p_value);

bool gls_glc_measurement_notify(uint8_t conn_id, uint8_t index);

bool gls_glc_measurement_context_notify(uint8_t conn_id, uint8_t index);

bool gls_racp_response(uint8_t conn_id, uint8_t rsp_code);

bool gls_racp_num_response(uint8_t conn_id, uint16_t num);

void gls_prepare_new_record(void);

void gls_push_new_record(void);

bool gls_report_records_task(uint8_t conn_id);

void user_face_time(TIMESTAMP time_in, int16_t time_offset, TIMESTAMP time_out);

void gls_abort_racp_procedure(void);

rtk_bt_err_t glucose_srv_add(void);

void gls_disconnect(uint8_t conn_id);
