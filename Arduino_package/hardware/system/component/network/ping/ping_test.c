...
Recv_retry:
		recv_retry_time = xTaskGetTickCount();
		passing_time = (int)((recv_retry_time - ping_time) * portTICK_RATE_MS);
		if (passing_time < ping_timeout) {
			recv_retry_timeout = ping_timeout - passing_time;
			setsockopt(ping_socket, SOL_SOCKET, SO_RCVTIMEO, &recv_retry_timeout, sizeof(recv_retry_timeout));
		}

		if ((passing_time < ping_timeout) && (recvfrom(ping_socket, reply_buf, reply_size, 0, (struct sockaddr *) &from_addr, (socklen_t *) &from_addr_len)
											  >= (int)(sizeof(struct ip_hdr) + sizeof(struct icmp_echo_hdr)))) {

			if (from_addr.sin_addr.s_addr != to_addr.sin_addr.s_addr) { // to prevent recv wrong-IP packets in concurrent mode
				goto Recv_retry;
			}
			iphdr = (struct ip_hdr *)reply_buf;
			pecho = (struct icmp_echo_hdr *)(reply_buf + (IPH_HL(iphdr) * 4));
			reply_type = pecho->type;

			if (reply_type != ICMP_ER) { // to prevent recv ICMP_ECHO during ping tx and rx simultaneously in station mode
				goto Recv_retry;
			}
