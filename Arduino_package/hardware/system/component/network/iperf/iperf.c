...
extern int wifi_fetch_phy_statistic(rtw_phy_statistics_t *phy_statistic);
		...
		while (((end_time - start_time) <= (configTICK_RATE_HZ * iperf_data.time)) && (!g_tcp_terminate)) {
			...
			if ((iperf_data.report_interval != DEFAULT_REPORT_INTERVAL) && ((end_time - report_start_time) >= (configTICK_RATE_HZ * iperf_data.report_interval))) {
				...
				if ((end_time - start_time) > configTICK_RATE_HZ * (iperf_data.time - 1)) {
					...
					printf("\n\r%s: Send %d KBytes in %d ms, %d Kbits/sec", __func__, (int)(report_size / KB), (int)(end_time - report_start_time),
						   (int)((report_size * 8) / (end_time - report_start_time)));
					if (iperf_data.extra_info) {
						int bcn_rssi = 0;
						rtw_phy_statistics_t phy_statistics;
						rltk_wlan_get_beacon_rssi(&bcn_rssi);
						wifi_fetch_phy_statistic(&phy_statistics);
						u8 tx_rate = 0;
						rltk_wlan_get_tx_rate(&tx_rate);
						printf("\n\r%s: bcn_rssi: %d, data_rssi: %d, tx_rate: %s", __func__, bcn_rssi, phy_statistics.rssi, DATA_RATE[tx_rate]);
					}
				}
		...
		if ((iperf_data.report_interval != DEFAULT_REPORT_INTERVAL) && ((end_time - report_start_time) >= (configTICK_RATE_HZ * iperf_data.report_interval))) {
			if ((end_time - start_time) > configTICK_RATE_HZ * (tcp_client_data.time - 1)) {
				...
				if (iperf_data.extra_info) {
					...
					wifi_fetch_phy_statistic(&phy_statistics);
					u8 rx_rate = 0;
					rltk_wlan_get_rx_rate(&rx_rate);
					printf("\n\r%s: bcn_rssi: %d, data_rssi: %d, rx_rate: %s", __func__, bcn_rssi, phy_statistics.rssi, DATA_RATE[rx_rate]);
				}
			}
...
void cmd_tcp(int argc, char **argv)
{
	...
	while (argv_count <= argc) {
		...
		if (argv_count == 2) {
			...
			if (strcmp(argv[argv_count - 1], "-d") == 0) {
				...
				argv_count += 2;
			} else if (strcmp(argv[argv_count - 1], "-e") == 0) {
				if (argc < (argv_count + 1)) {
					goto Exit;
				}
				if (tcp_server_data.start) {
					tcp_server_data.extra_info = 1;
				} else if (tcp_client_data.start) {
					tcp_client_data.extra_info = 1;
				} else {
					goto Exit;
				}
				argv_count += 2;
	...
	if (tcp_client_data.start && (NULL == g_tcp_client_task)) {
		...
		int priority = tskIDLE_PRIORITY + 4;
		if (g_tcp_bidirection == 1) {
			priority = tskIDLE_PRIORITY + 5;
		}

		if (xTaskCreate(tcp_client_handler, "tcp_client_handler", BSD_STACK_SIZE, NULL, priority, &g_tcp_client_task) != pdPASS) {
			printf("\n\rTCP ERROR: Create TCP client task failed.");
		} else {
...
Exit:
	...
	printf("  \r     -e  #          enable tx_rate/rx_rate/data_rssi/bcn_rssi information\n");
	printf("\n\r   Example:\n");
	...
	while (argv_count <= argc) {
		...
		if (argv_count == 2) {
			...
			if (strcmp(argv[argv_count - 1], "-b") == 0) {
				...
				if (udp_server_data.start) {
					udp_server_data.extra_info = 1;
				} else if (udp_client_data.start) {
					udp_client_data.extra_info = 1;
				} else {
					goto Exit;
				}
				argv_count += 2;
