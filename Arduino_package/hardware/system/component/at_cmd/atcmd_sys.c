...
TaskStatus_t *find_status(int id, TaskStatus_t *status, int cnt)
{
	for (int i = 0; i < cnt; i++) {
...
void detect_deleted_task_start(delta_status_t *delta, int cnt)
{
	for (int i = 0; i < cnt; i++) {
...
void detect_deleted_task_stop(delta_status_t *delta, int cnt)
{
	for (int i = 0; i < cnt; i++) {
		if (delta[i].deleted == 1) {
			// record should removing
			memset(&delta[i], 0, sizeof(delta_status_t));
		}
	}
}
...
static int usage_comparator(const void *pa, const void *pb)
{
	delta_status_t *a = (delta_status_t *)pa;
	delta_status_t *b = (delta_status_t *)pb;
	int diff = a->running_tick - b->running_tick;
...
static void update_status(void)
{
	...
	task_status.curr_cnt = task_status.curr_cnt > TASK_CNT ? TASK_CNT : task_status.curr_cnt;
	task_status.curr_cnt = uxTaskGetSystemState(task_status.curr, task_status.curr_cnt, NULL);

	// update ts
	task_status.last_ts = task_status.curr_ts;
	task_status.curr_ts = xTaskGetTickCount();

	detect_deleted_task_start(task_status.delta, TASK_CNT);
	// update delta
	if (task_status.last_cnt > 0) {
		for (int i = 0; i < task_status.curr_cnt; i++) {
			...
			// handle delete task
			/*
			if (task_status.curr[i].eCurrentState == eDeleted ) {
				delta_status_t *deltaone = find_delta(task_status.curr[i].xTaskNumber, task_status.delta, TASK_CNT);
				if (!deltaone) {
					memset(deltaone, 0, sizeof(delta_status_t));
				}
			}
			*/
		...
		task_status.delta_ts = task_status.curr_ts - task_status.last_ts;
	}
	detect_deleted_task_stop(task_status.delta, TASK_CNT);

	// sort
	qsort(task_status.delta, TASK_CNT, sizeof(delta_status_t), usage_comparator);
}

void print_delta(int max_task)
{
	int total = 0;
	int deleted_ts = 0;
	int shown_ts = 0;
	int max_cnt = max_task;

	char c_state[5] = {'R', 'r', 'b', 's', 'D'};
	printf("%c%c%c%c", 0x1B, 0x5B, 0x32, 0x4A);
	printf("%-20s\t%6s\t%6s\t%s\t%s\n\r", "task name", "state", "prio", "CPU%", "ID");

	for (int i = 0; i < TASK_CNT; i++) {
		if (task_status.delta[i].task != NULL) {
			total += task_status.delta[i].running_tick;
		}
	}

	if (total < task_status.delta_ts) {
		total = task_status.delta_ts;
	}

	for (int i = 0; i < TASK_CNT; i++) {
		if (task_status.delta[i].task != NULL) {
			printf("%-20s\t%6c\t%6d\t%2.2f\t%d\n\r", task_status.delta[i].name, c_state[task_status.delta[i].state], task_status.delta[i].priority,
				   (float)task_status.delta[i].running_tick * 100 / (float)total, task_status.delta[i].task_id);
			shown_ts += task_status.delta[i].running_tick;
			max_cnt --;
			if (max_cnt <= 0) {
				break;
			}
	...
	deleted_ts = (total - shown_ts);

	if (deleted_ts > 0) {
		printf("%-20s\t%6c\t%6d\t%2.2f%\t%d\n\r", "Closed or Hided", '?', -1, (float)deleted_ts * 100 / (float)total, -1);
	}

}

static void *top_exit_sema;
void top_thread(void *param)
{
	unsigned int top_param = (unsigned int)param;

	int top_period = (top_param >> 16) & 0xffff;
	int top_max_task = top_param & 0xffff;
	...
	while ((xSemaphoreTake((SemaphoreHandle_t)top_exit_sema, top_period * 1000) == pdFALSE)) {
		update_status();
		print_delta(top_max_task);
	}
...
void top_run(int top_mode, int top_period, int top_max_task)
{
	unsigned int top_param = ((top_period & 0xffff) << 16) | (top_max_task & 0xffff);
	switch (top_mode) {
	case 0:
		if (top_exit_sema)	{
			break;
		...
		if (top_exit_sema) {
			xTaskCreate(top_thread, ((const char *)"top"), 4096, (void *)top_param, tskIDLE_PRIORITY + 1, NULL);
		}
		break;
	case 1:
		if (top_exit_sema) {
...
void fTOP(void *arg)
{
	...
	top_mode = strtol(argv[1], NULL, 10);
	if (top_mode == 0 && (argc >= 2 && argc <= 4)) {
		if (argc >= 3) {
			top_period = strtol(argv[2], NULL, 10);
		}
		if (argc == 4) {
			top_max_task = strtol(argv[3], NULL, 10);
		}
	}

	if ((top_mode == 0 && argc > 4) || (top_mode != 0 && argc > 2)) {
		printf("Usage: \n\r");
		printf("\ttop[=0],[period],[max display count]   -  periodically update start\n\r");
		printf("\ttop=1, stop top=0\n\r");
		printf("\ttop=2, oneshoot start\n\r");
		printf("\ttop=3, oneshoot stop\n\r");
		return;
	}

	top_run(top_mode, top_period, top_max_task);
}
...
//	uint32_t ability = 0;
	//char buf[64] = {0};
	const char *compile_sdk_ver = ".4.0";
	const char *compile_date = RTL_FW_COMPILE_DATE;
...
//		strcat(buf, "m");
	...
	if (sw_version[0] != 0xff) {
		...
		AT_PRINTK("[ATS?]: SW VERSION: v%s%s", compile_sdk_ver, compile_date);
	}
