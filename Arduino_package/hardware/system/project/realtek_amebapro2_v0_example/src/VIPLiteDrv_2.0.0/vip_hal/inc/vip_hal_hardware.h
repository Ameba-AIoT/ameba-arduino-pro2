...
#ifndef _VIP_HAL_HARDWARE_H
#define _VIP_HAL_HARDWARE_H
#include <vip_lite_config.h>


typedef enum _viphal_hardware_trsp2_type {
	VIPHAL_HARDWARE_TRSP2_NONE = 0,
	VIPHAL_HARDWARE_TRSP2_DDR_TO_VIPSRAM = 1,
	VIPHAL_HARDWARE_TRSP2_VIPSRAM_TO_DDR = 2,
	VIPHAL_HARDWARE_TRSP2_MAX
} viphal_hardware_trsp2_type;
...
@param type, viphal_hardware_trsp2_type.
*/
vip_status_e viphal_hardware_trsp2_instr(
	vip_uint8_t *buffer,
	vip_uint32_t *size,
	vip_uint32_t src_physical,
	vip_uint32_t dst_physical,
	vip_uint32_t data_size,
	viphal_hardware_trsp2_type type
);
...
*/
vip_status_e viphal_hardware_trsp2_loadstate(
	vip_uint8_t *buffer,
	vip_uint32_t *size,
...
*/
vip_status_e viphal_hardware_multivip_sync(
	vip_uint32_t *buffer,
	vip_uint32_t *size,
...
typedef enum _gcvip_probe_proflie_cmd {
	VIPHAL_PROBECMD_BEGIN = 0,
	VIPHAL_PROBECMD_PAUSE = 1,
	VIPHAL_PROBECMD_RESUME = 2,
	VIPHAL_PROBECMD_END = 3,
} viphal_probe_proflie_cmd_e;

typedef struct _viphal_probe_param {
	/* the physical address to store counter */
	vip_uint32_t physical;
	/* how many cores in cluster */
	vip_uint32_t clusterIDWidth;
	/* the type of cmd */
	viphal_probe_proflie_cmd_e cmd;
} viphal_probe_param;
/*
...
*/
vip_status_e viphal_hardware_probe_cmd(
	vip_uint32_t *buffer,
	vip_uint32_t *size,
	vip_uint32_t core_count,
	vip_uint32_t *core_id,
	viphal_probe_param *probe_param
);
#endif
#endif
