...
static char* readline(SIM_FILE *input)
{
	int len;

	if(sim_fgets(line,max_line_len,input) == NULL)
		return NULL;
	...
	while(strrchr(line,'\n') == NULL)
	{
		...
		len = (int) strlen(line);
		if(sim_fgets(line+len,max_line_len-len,input) == NULL)
			break;
	}
	return line;
}
...
// is used
//
#define FSCANF(_stream, _format, _var) do{ if (sim_fscanf(_stream, _format, _var) != 1) return false; }while(0)
bool read_model_header(SIM_FILE *fp, svm_model* model)
{
...
{
	SIM_FILE *fp = sim_fopen(model_file_name,"rb");
	...
	if (sim_ferror(fp) != 0 || sim_fclose(fp) != 0)
		return NULL;
